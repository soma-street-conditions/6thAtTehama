...
# 7. Helper: VERINT IMAGE CRACKER (JSON Unwrap)
    ...
    try:
        ...
        if r_list.status_code == 200:
            ...
            if filename_str:
                ...
                if debug_mode: logs.append(f"✅ Step 3 SUCCEEDED!")
            else:
        ...
        # STEP 5: DOWNLOAD & UNWRAP JSON
        download_payload = p2.copy()
        ...
        if r_image.status_code == 200:
            # IT IS A JSON RESPONSE, NOT RAW BYTES!
            try:
                ...
                if 'data' in response_json and 'txt_file' in response_json['data']:
                    ...
                    if debug_mode: logs.append(f"✅ Step 5 Unwrapped: {len(image_bytes)} bytes")
                    return image_bytes, logs
                else:
                    if debug_mode: logs.append(f"❌ Step 5 JSON missing 'txt_file'")
                    return None, logs
            except Exception as e:
                if debug_mode: logs.append(f"❌ Step 5 Failed to parse JSON/Base64: {str(e)}")
                return None, logs
        else:
